...
def test_carregar_dados_json_estrutura_incorreta(tmp_path):
    dados_teste = {
        "Código": "123"
    }  # Estrutura inesperada (falta chave "Empresas Contratadas")
    ...
    dados_carregados = carregar_dados_json(caminho_arquivo_json)
    assert (
        dados_carregados == dados_teste
    )  # Pode mudar dependendo do que a função deve fazer
