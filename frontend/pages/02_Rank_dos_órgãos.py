...
def load_csv_data():
    ...
    file_path = os.path.join(
        os.path.dirname(__file__), "..", "contratos_ordenados_completo.csv"
    )
...
with st.container():
    st.subheader(
        "PAINEL INTERATIVO SOBRE OS GASTOS GOVERNAMENTAIS COM DISPENSA DE LICITAÇÃO"
    )
...
df_filtered = (
    df_ordenado[df_ordenado["Ano da Compra"] == ano] if ano != "Todos" else df_ordenado
)

# Filtra por órgão
orgaos_unicos_lista = ["Todos"] + list(df_filtered["Órgão Entidade"].unique())
orgaos_unicos = st.sidebar.selectbox(
    "Órgão Governamental Contratante", orgaos_unicos_lista
)
...
palavra_filtro = st.sidebar.text_input(
    "Filtrar por Palavras-Chave na Coluna 'Objeto da Compra': coffee break, lanche, abastecimento e etc."
)
if palavra_filtro:
    df_filtered = df_filtered[
        df_filtered["Objeto da Compra"].str.contains(palavra_filtro, case=False)
    ]


# Filtra pelo nome da empresa contratada
empresa_opcoes = ["Todos"] + list(df_filtered["Empresa Contratada"].unique())
contrato_empresa_opcoes_filtro = st.sidebar.selectbox(
    "Filtrar pelo nome da Empresa Contratada", empresa_opcoes
)
if contrato_empresa_opcoes_filtro != "Todos":
    df_filtered = df_filtered[
        df_filtered["Empresa Contratada"] == contrato_empresa_opcoes_filtro
    ]
...
valor_min_selecionado = st.sidebar.number_input(
    "Valor Contratado - De: (em R$):",
    min_value=valor_min,
    max_value=valor_max,
    value=valor_min,
)
valor_max_selecionado = st.sidebar.number_input(
    "Valor Contratado - Até: (em R$):",
    min_value=valor_min,
    max_value=valor_max,
    value=valor_max,
...
df_filtered = df_filtered[
    (df_filtered["Valor Recebido"] >= valor_min_selecionado)
    & (df_filtered["Valor Recebido"] <= valor_max_selecionado)
]
...
pagina = st.sidebar.number_input(
    f"Nº da página do total de {num_paginas} páginas",
    min_value=1,
...
df_paginado = df_filtered.iloc[inicio:fim].sort_values(
    by="Valor Recebido", ascending=False
)

# Adicionar a coluna 'Órgão Entidade + Código' para o gráfico
df_paginado["Órgão Entidade + Código"] = (
    df_paginado["Órgão Entidade"] + " " + df_paginado["Código"]
)
...
# Gráfico 2
df_grouped_chart2 = (
    df_filtered.groupby(["Órgão Entidade"])["Valor Recebido"]
    .sum()
    .reset_index()
    .sort_values(by="Valor Recebido", ascending=False)
)
# Selecionar apenas os 20 primeiros
df_grouped_chart2_top20 = df_grouped_chart2.head(15)
fig_orgao2 = px.bar(
    df_grouped_chart2_top20,
    x="Órgão Entidade",
    y="Valor Recebido",
    color="Órgão Entidade",
    title="Órgãos Campeões",
)
...
df["Órgão Entidade Normalizado"] = df["Órgão Entidade"].apply(
...
if search_term:
    ...
    matched_organs = df[
        df["Órgão Entidade Normalizado"].str.contains(search_term_normalized)
    ]["Órgão Entidade"].unique()
    ...
    if len(matched_organs) == 0:
        ...
        if selected_organ:
            ...
            df_grouped = (
                df_filtered.groupby(["Ano da Compra", "Órgão Entidade"])[
            ...
            fig_timeline = px.line(
                ...
                color="Órgão Entidade",
                title="Evolução dos Gastos Anuais por Órgão",
            )
            ...
            with st.container():
                st.write(
                    f'<h3><u style="color:white;">Valor anual gasto em dispensa de licitação por:</u> {selected_organ}</h3>',
                    unsafe_allow_html=True,
                )
                for _, row in df_grouped.iterrows():
                    st.markdown(
                        f"<p style='font-size:16px;'><u style='color:white;'>Ano {row['Ano da Compra']}:</u> R$ {row['Valor Total Homologado']:,.2f}</p>",
                        unsafe_allow_html=True,
                    )
                st.write(
                    "A primeria versão do Portal Nacional de Contratações Públicas [(PNCP)](https://www.gov.br/pncp/pt-br) somente foi lançada em Agosto de 2021 portanto, os dados desse ano podem estar incompletos."
                )
