...
def lista_cnpjs(arquivo):
    with open(arquivo, "r", encoding="utf-8") as file:
        ...
        lista = [
            ...
            if (linha[-3].isnumeric() and len(linha[-3]) == 14)
        ]  # faz uma lista com o elemento -3, que corresponde ao cnpj
...
def lista_3_listas(lista):
    tamanho = len(lista) // 3
    return [
        lista[i * tamanho : (i + 1) * tamanho] for i in range(3)
    ]  # cria uma lista contendo 3 listas (cada uma com tamanhos semelhantes)


if __name__ == "__main__":
    # retorna a lista das empresas contratadas
    CNPJs = lista_cnpjs(
        "frontend/x_empresas_contratadas.csv"
    )  # inicializa a primeira função
