...
def expand_contracts(data):
    ...
    for entry in data:
        base_contract = {
            key: value for key, value in entry.items() if key != "Empresas Contratadas"
        }
        ...
        while f"Empresa Contratada -{i}" in empresas_contratadas:
            empresa_info = {
                "Empresa Contratada": empresas_contratadas.get(
                    f"Empresa Contratada -{i}"
                ),
...
def read_file_cnpj():
    ...
    cadastros_cnpj["Data de Início da Atividade"] = pd.to_datetime(
        cadastros_cnpj["Data de Início da Atividade"]
    )

    # Extrair o ano e inserir a nova coluna imediatamente após 'Data de Início da Atividade'
    cadastros_cnpj.insert(
        cadastros_cnpj.columns.get_loc("Data de Início da Atividade") + 1,
        "Ano de Início da Atividade",
        cadastros_cnpj["Data de Início da Atividade"].dt.year,
    )
...
def verificar_diferenca(df):
    ...
    contratos_fila = df
    contratos_fila["Valor Total Homologado Ajustado"] = contratos_fila[
        "Valor Total Homologado"
    ].astype(float)
    contratos_fila["Valor Total Estimado Ajustado"] = contratos_fila[
        "Valor Total Estimado"
    ]
    ...
    for code in contratos_fila["Código"].unique():
        ...
        contratos_fila.loc[first_index, "Valor Total Estimado Ajustado"] = (
            contratos_fila.loc[first_index, "Valor Total Estimado"]
        )
        contratos_fila.loc[first_index, "Valor Total Homologado Ajustado"] = (
            contratos_fila.loc[first_index, "Valor Total Homologado"]
        )

    # Remover as colunas originais e renomear as ajustadas
    contratos_fila = contratos_fila.drop(
        columns=["Valor Total Estimado", "Valor Total Homologado"]
    )
    contratos_fila = contratos_fila.rename(
        columns={
    ...
    contratos_fila["Valor Total Homologado Ajustado"] = contratos_fila.groupby(
        "Código"
    )["Valor Total Homologado"].transform(lambda x: x.where(x.index == x.idxmax(), 0))

    # Calcular a soma dos valores recebidos por Código
    soma_valor_recebido = (
        contratos_fila.groupby("Código")["Valor Recebido"].sum().reset_index()
    )
    ...
    contratos_fila = contratos_fila.merge(
        soma_valor_recebido[["Código", "Valor Recebido_Soma"]], on="Código", how="left"
    )

    # Calcular a diferença entre o Valor Total Homologado e a soma dos Valores Recebidos
    contratos_fila["Diferença"] = (
        contratos_fila["Valor Total Homologado"] - contratos_fila["Valor Recebido_Soma"]
    ).round(2)
...
def arredonda_valores(df):
    ...
    df_arredondados[
        ["Valor Total Estimado", "Valor Total Homologado", "Valor Recebido"]
    ] = df_arredondados[
        ["Valor Total Estimado", "Valor Total Homologado", "Valor Recebido"]
    ].round(
        2
...
contratos_merged = merge(
    contratos_fila, contratos_cnpj
)  # avaliar a necessidade de puxar o cadastro das empresas
...
contratos_merged.to_csv(
    "frontend/contratos_ordenados_completo.csv", index=False
)  # ajustei estava ('contratos_ordenados_completos.csv')
