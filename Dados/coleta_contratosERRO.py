...
def get_resultados(contrato):
    ...
    for tentativa in range(tentativas): # Ele irá tentar 5 vezes fazer o request novamente em caso de falha
        ...
        try:    
                    ...
                    for tent in range(tentativas):
                        try:
                            ...
                            if (tent < tentativas - 1):
                                time.sleep(tempo_espera) # Espera o wait time para fazer a próxima requisição
                                tempo_espera *= 2 # Dobra o tempo de espera para a próxima tentativa de requisição (caso exista)
                                print(f"Quantidade de itens do contrato {contrato['numeroControlePNCP']} : {nItens}\nTentativa número {tent} do item numero {i} do request para pegar os resultados de cada item do contrato: {contrato['numeroControlePNCP']}")
                            else:
                                logger.error(f"Erro ao obter contrato: {contrato['numeroControlePNCP']} devido à inconsistência no item número {i}. Quantidade total de itens no contrato: {nItens}\nErro: {e}")
            ...
            if (tentativa < tentativas - 1):
                ...
                print(f"Tentativa número {tentativa} do request para saber o número de itens de cada contratação do contrato: {contrato['numeroControlePNCP']}")
            else:
                print(f"Erro ao obter contrato: {contrato['numeroControlePNCP']}")
                return None
...
with open("contratos_OFICIAL_versao3 copy.json","a",encoding='utf-8') as f:
    f.write("\n")
        ...
        for tentativa in range(tentativas):
            try:
                ...
                if (tentativa < tentativas - 1):
                    time.sleep(wait_time) # Espera o wait time para fazer a próxima requisição
                    wait_time *= 2 # Dobra o tempo de espera para a próxima tentativa de requisição (caso exista)
                    print(f"Tentativa número {tentativa} do request inicial do contrato: {contrato}")
                else:
